...
無實體螢幕並使用 VDD 時，如果擷取到的 PNG 與同一個 Roblox 程序上次**成功傳送**的畫面完全相同，定時截圖會略過並在 60 秒後再檢查，避免反覆傳同一張圖。狀態顯示在設定視窗；比對紀錄放在本機 `last-screenshot.json`（不含圖片或 Webhook，已排除 Git）。手動 F8 與斷線通知不受此限制。這只能辨識重複影像，不能強制 VDD 或 Roblox 產生新畫面；如果狀態一直顯示畫面相同，請檢查 VDD 是否持續輸出、Roblox 視窗是否仍在更新。
